...
{
    $public_atts = lAtts( array( 'login_invite'  => gTxt( 'login_to_textpattern' )
                               , 'logout_invite' => ''
                               , 'show_change'   => '1'
                               , 'show_reset'    => '1'
                               , 'tag_invite'    => ''
                               , 'login_label'   => gTxt( 'log_in_button' )
                               , 'logout_label'  => gTxt( 'logout' )
                               , 'logout_type'   => 'button'
                               , 'tag_error'     => 'span'
...
{
    extract( lAtts( array ( 'label' => gTxt( 'stay_logged_in' )
                          )
                  + _cbe_fa_format()
                  , $atts
                  )
...
function cbe_frontauth_edit_article( $atts )
{
    ...
    extract( lAtts( array ( 'label' => gTxt( 'edit' )
                          , 'type'  => 'button'
                          )
                  + _cbe_fa_format()
                  , $atts
...
{
    extract( lAtts( array ( 'label'     => gTxt( $field )
                          , 'label_sfx' => ''
                          )
                  + _cbe_fa_format()
                  , $atts
...
/*
    ...
    $out .= fInput( ($field == 'name')    ? 'text'     : 'password'
                  , ($field == 'name')    ? 'p_userid' : 'p_password'
                  , ($field == 'name')    ? $name      : ''
                  , (!$wraptag && $class) ? $class     : ''
                  , '', '', '', '', $field ) ;
...
function _cbe_fa_inout_process( $inout, $atts, $thing = '' )
{
    $plus_atts = ($inout == 'logout' )
               ...
               : array( 'show_stay'   => '0'
                      , 'show_reset'  => '1'      ) ;
    $public_atts = lAtts( array ( 'invite'     => gTxt( ($inout == 'login') ? 'login_to_textpattern' : '' )
                                , 'tag_invite' => ''
                                , 'label'      => gTxt( ($inout == 'login') ? 'log_in_button' : 'logout' )
                                , 'form'       => ''
...
function _cbe_fa_button( $atts )
{
    ...
    if( ! $label and ! ($label = cbe_frontauth( array( 'init' => '0', 'value' => $type.'_label' ) )) )
        $label = gTxt( ($type == 'logout' || $type == 'edit' ) ? $type : 'log_in_button' ) ;
...
//    $adm_path = $pub_path . substr(strrchr(txpath, "/"), 1) . '/' ;
...
function cbe_frontauth( $atts, $thing = null )
{
    ...
    $def_atts = array( 'form'          => ''
                     , 'tag_invite'    => ''
                     , 'show_login'    => '1'
                     , 'login_invite'  => gTxt( 'login_to_textpattern' )
                     , 'login_form'    => ''
                     , 'login_label'   => gTxt( 'log_in_button' )
                     , 'login_with'    => 'auto'
                     , 'login_redir'   => ''
...
//    extract( psa( array( 'p_userid', 'p_password', 'p_stay', 'p_logout' ) ) ) ;
    $logout = gps( 'logout' ) ;
    ...
    if( $p_userid && $p_password )
    {
        ...
        $txp_user = _cbe_fa_auth( $login_redir, 0, $p_userid, $p_password, $p_stay ) ;
    ...
    if( $txp_user === false )
    {
        ...
        if( $show_login )
        {
            ...
            {   // Resetting password in progress
                $invite = is_callable( '_cbe_mb_gTxt' ) ? _cbe_mb_gTxt( 'resetpassword' ) : '' ;
    ...
    else
    {
        ...
        if( $show_logout )
        {
            ...
            {   // Changing password in progress
                $invite = is_callable( '_cbe_mb_gTxt' ) ? _cbe_mb_gTxt( 'change_password' ) : '' ;
